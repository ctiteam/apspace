...
export class StudentDashboardPage implements OnInit {
  ...
  getUpcomingClasses(intake: string): Observable<EventComponentConfigurations[]> {
    ...
    return this.studentTimetableService.get().pipe(
      ...
      map((timetables: StudentTimetable[]) => {
        let timetableEventMode: EventComponentConfigurations[] = [];
        timetables.forEach((timetable: StudentTimetable) => {
          const secondsDiff = this.getSecondsDifferenceBetweenTwoDates(moment(timetable.TIME_FROM, 'HH:mm A').toDate(), moment(timetable.TIME_TO, 'HH:mm A').toDate());
          let classPass = false;
    ...
    return this.ws.get<ConsultationHour[]>('/iconsult/upcomingconstu', true).pipe(
      ...
      map(upcomingConsultations => {
        upcomingConsultations.forEach(upcomingConsultation => {
          ...
          const secondsDiff = this.getSecondsDifferenceBetweenTwoDates(moment(upcomingConsultation.starttime, 'HH:mm A').toDate(), moment(upcomingConsultation.endtime, 'HH:mm A').toDate());
  ...
  eventIsToday(eventDate: Date, todaysDate: Date) {
    return (eventDate.getFullYear() === todaysDate.getFullYear() && eventDate.getMonth() === todaysDate.getMonth() && eventDate.getDate() === todaysDate.getDate());
  }
  ...
  getupcomingExams(intake: string, todaysDate: Date): Observable<EventComponentConfigurations[]> {
    ...
    return this.ws.get<ExamSchedule[]>(
      ...
      map(examsList => {
        let examsListEventMode: EventComponentConfigurations[] = [];
  ...
  getUpcomingHoliday(date: Date): Observable<EventComponentConfigurations[]> {
    return this.ws.get<Holidays>('/transix/holidays/filtered/students', true).pipe(
      map(holidays => {
        return holidays.holidays.sort((holidayA, holidayB) => {
          return moment(holidayA.holiday_start_date, 'YYYY-MM-DD').toDate() > moment(holidayB.holiday_start_date, 'YYYY-MM-DD').toDate()
            ? 1
            : moment(holidayA.holiday_start_date, 'YYYY-MM-DD').toDate() < moment(holidayB.holiday_start_date, 'YYYY-MM-DD').toDate()
              ? -1
              : 0
        })
      }
      ),
      map(holidays => holidays.find(h => date < new Date(h.holiday_start_date)) || {} as Holiday),
      map(holiday => {
        let examsListEventMode: EventComponentConfigurations[] = [];
    ...
    this.modulesWithLowAttendance$ = this.ws.get<Attendance[]>(url, true).pipe(
      tap(attendanceData => {
        ...
        if (attendanceData.length > 0) {
          let totalClasses = attendanceData.reduce((a, b) => a + b.TOTAL_CLASSES, 0);
          let totalAbsentClasses = attendanceData.reduce((a, b) => a + b.TOTAL_ABSENT, 0);
